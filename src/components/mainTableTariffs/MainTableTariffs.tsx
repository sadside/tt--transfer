...
const MainTableTariffs: FC<MainTableTariffsProps> = ({}) => {
	...
	const itemsHeader = ['Название', 'Город', 'Активный', 'Действует до', 'Изменен', 'Тип']
					...
					<thead>
						<tr>
							<th className={s.checkbox} style={{ width: '4%' }}>
								<input type='checkbox' onChange={selectAll} checked={isSelectAll} />
							</th>
							{itemsHeader.map((item, index) => (
								<th className={s.item} key={index} align={'center'} style={{ width: '16%' }}>
									{item}
								</th>
							))}
						</tr>
