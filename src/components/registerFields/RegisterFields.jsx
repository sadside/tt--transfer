	...
	return (
		<div className='register-fields-wrap'>
			...
			<div className='register-fields-description'>
				Создайте аккаунт Трансфер и получите все необходимые инструменты эффективного управления
				бизнесом! У вас уже есть аккаунт Трансфер?{' '}
				<Link to='/login' style={{ textDecoration: 'none', color: '#6d9bee' }}>
					Войти в аккаунт
				</Link>
			</div>
			<div className='register-fields-inputs'>
				<form action='' className={'reg-form'} onSubmit={handleSubmit(onSubmit)}>
					<div className='' ref={ref}>
						<div>
							<div className='role-class-select' onClick={() => setIsShow(!isShow)}>
								<div>{activeItem}</div>
						...
						<AnimatePresence>
							{errors?.surname && (
								<motion.div
									...
									<div className='error-component'>{errors.surname.message}</div>
								</motion.div>
							)}
						</AnimatePresence>
					</div>
						...
						<AnimatePresence>
							{errors?.patronymic && (
								<motion.div
									...
									transition={{ type: 'Tween' }}
									style={{ overflow: 'hidden' }}>
									<div className='error-component'>{errors.patronymic.message}</div>
								</motion.div>
							)}
						</AnimatePresence>
					</div>
					...
					<AnimatePresence>
						{errors?.firstPassword && (
							<motion.div
								...
								transition={{ type: 'Tween' }}
								style={{ overflow: 'hidden' }}>
								<div className='error-component'>{errors.firstPassword.message}</div>
							</motion.div>
						)}
					</AnimatePresence>
					<input
					...
					<AnimatePresence>
						{errors?.secondPassword && (
							<motion.div
								...
								transition={{ type: 'Tween' }}
								style={{ overflow: 'hidden' }}>
								<div className='error-component'>{errors.secondPassword.message}</div>
							</motion.div>
						)}
					</AnimatePresence>
